...
BEGIN
  FOR mgr IN SELECT id, created_at FROM public.managers LOOP
    ...
    LOOP
      ...
      -- Undo each post-race transfer in reverse-chronological order:
      -- remove new_driver_id, add old_driver_id
      FOR t IN
      ...
      -- Sum race points for drivers in historical team
      ...
      -- Captain bonus: if captain driver scored in this race
